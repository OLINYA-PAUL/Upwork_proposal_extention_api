...
const FREE_TRIAL_LIMIT = 10; // 10 proposals free trial
...
export class UsageLimiterInterceptor implements NestInterceptor {
  ...
  async intercept(
    ...
    // ADMIN role — bypass all limits
    if (user.role === 'ADMIN') {
      this.logger.log(`Admin bypassing usage limits — user: ${user.sub}`);
      return next.handle();
    }
    ...
    // ── COUNT ALL PROPOSALS GENERATED BY USER ──────────────
    const totalProposalsGenerated = await this.prisma.proposal.count({
    ...
    if (trialEnabled) {
      ...
      if (trialAllowed) {
        // ── TRIAL STILL ACTIVE ───────────────────────────────
        if (totalProposalsGenerated < FREE_TRIAL_LIMIT) {
          const remaining = FREE_TRIAL_LIMIT - totalProposalsGenerated;

          // Store fingerprint on first use
          if (totalProposalsGenerated === 0 && dbUser.fingerprint) {
            await this.prisma.deviceFingerprint.upsert({
              where: { fingerprint: dbUser.fingerprint },
              update: {},
              create: {
            ...
            this.logger.log(
              `Trial started — user: ${dbUser.id} — ${FREE_TRIAL_LIMIT} free proposals`,
            );
          } else {
            await this.prisma.user.update({
              where: { id: dbUser.id },
          ...
          // Attach remaining count to request for response
          request.trialRemaining = remaining - 1;

          this.logger.log(
            `Trial proposal — user: ${dbUser.id} — ${remaining - 1} remaining after this`,
          );
        ...
        // ── TRIAL EXHAUSTED ──────────────────────────────────
        this.logger.log(
          `Trial exhausted — user: ${dbUser.id} — ${totalProposalsGenerated} proposals used`,
        );
      }
    }

    // ── TRIAL DISABLED OR EXHAUSTED OR ABUSED ─────────────
    // STARTER plan — monthly limit
    if (dbUser.plan === Plan.STARTER) {
      await this.enforceMonthlyLimit(dbUser, now);
      return next.handle();
    }

    // FREE plan — 1 per 24 hours
    await this.enforceFreeDailyLimit(dbUser, now);
    return next.handle();
  }
  ...
  private async enforceFreeDailyLimit(dbUser: any, now: Date): Promise<void> {
    if (dbUser.lastRequestAt) {
      ...
      if (timeSinceLastRequest < COOLDOWN_MS) {
        ...
        throw new ForbiddenException({
          message:
            `Your free trial has ended. You are limited to 1 proposal per 24 hours. ` +
            `Next request available in ${timeMessage}.`,
          upgradeRequired: true,
          nextAvailableIn: remainingMs,
          trialRemaining: 0,
